...
public class PickupRepository : MonoBehaviour
{
    ...
    public static void SetHasBeenPickedUp(string pickupName, bool hasBeenPickedUp)
    {
        ...
        PlayerPrefs.SetInt(PICKUP_KEY(SceneManager.GetActiveScene().name + pickupName), state);
    }

    public static bool GetHasBeenPickedUp(string pickupName)
    {
        bool hasKey = PlayerPrefs.HasKey(PICKUP_KEY(SceneManager.GetActiveScene().name + pickupName));
        bool pickedUpState = PlayerPrefs.GetInt(PICKUP_KEY(SceneManager.GetActiveScene().name + pickupName)) >= 1;
