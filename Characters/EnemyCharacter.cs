...
public class EnemyCharacter : CharacterBase, ILogicUpdate, IPhysicsUpdate, IParriable
{
    ...
    {
        Vector2 directionToPlayer = aiVision.TargetPlayerCharacter.transform.position - playerCheck.position;
    ...
    public virtual bool CheckPlayerInMinAggroRange()
    {
        if (playerCheck == null || CheckGroundInRange(playerCheck.position, transform.right, enemyData.minAggroDistance))
            return false;
        if (AIVision && AIVision.TargetPlayerCharacter && AIVision.TargetPlayerCharacter.IsAlive())
            return Vector3.Distance(playerCheck.position, AIVision.TargetPlayerCharacter.transform.position) < enemyData.minAggroDistance;
        return false;
    }
    ...
    public virtual bool CheckPlayerInMaxAggroRange()
    {
        if (CheckGroundInRange(playerCheck.position, transform.right, enemyData.maxAggroDistance))
            return false;
        if (AIVision && AIVision.TargetPlayerCharacter && AIVision.TargetPlayerCharacter.IsAlive())
            return Vector3.Distance(playerCheck.position, AIVision.TargetPlayerCharacter.transform.position) < enemyData.maxAggroDistance;
        return false;
    }
    ...
    public Transform GetTargetTransform()
    {
        ...
        return AIVision.TargetPlayerCharacter.transform;
    }
